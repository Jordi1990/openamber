...
static const char *const TAG = "opentherm.slave";

void OpenthermSlaveHub::setup() {
  ESP_LOGI(TAG, "Setting up OpenTherm slave...");
  ...
  ESP_LOGI(TAG, "OpenTherm slave ready, listening for thermostat");
}
...
void OpenthermSlaveHub::loop() {
  ...
  if (now - this->last_status_log_ > 10000) {
    ...
    ESP_LOGD(TAG, "Status: mode=%s, rx=%u, tx=%u, err=%u",
  ...
  switch (this->protocol_->get_mode()) {
    case ProtocolMode::RECEIVED: {
      ...
      if (this->protocol_->get_message(request)) {
        ...
        ESP_LOGD(TAG, "RX [id=%d] type=%s hb=0x%02X lb=0x%02X (f88=%.1f) -> TX type=%s hb=0x%02X lb=0x%02X",
                 request.id,
    ...
    case ProtocolMode::ERROR_TIMEOUT:
      this->errors_++;
      ESP_LOGW(TAG, "Timeout error (total: %u, bit=%d, clock=%u, capture=0x%08" PRIX32 ") restarting listener",
    ...
    case ProtocolMode::ERROR_TIMER:
      this->errors_++;
      ESP_LOGW(TAG, "Timer error (total: %u, bit=%d, clock=%u), restarting listener", this->errors_,
...
void OpenthermSlaveHub::dump_config() {
  ESP_LOGCONFIG(TAG, "OpenTherm Slave:");
...
void OpenthermSlaveHub::process_request_(const OpenthermData &request) {
  ...
  switch (msg_id) {
    case MessageId::STATUS: {
      auto status = MasterStatus::from_hb(request.value_hb);
      this->publish_binary_(this->ch_enable_sensor_, status.ch_enable);
      this->publish_binary_(this->dhw_enable_sensor_, status.dhw_enable);
      this->publish_binary_(this->cooling_enable_sensor_, status.cooling_enable);
      break;
    }
