...
// ============================================================================
// OPENAMBER COMPONENT IMPLEMENTATION
// ============================================================================
// NOTE: This must be here (not in openamber_component.h) because it needs
// access to ESPHome's id() macro and the controller classes, which require
// the main ESPHome context.
...
OpenAmberComponent::OpenAmberComponent()
{
  ...
  hp_state_ = HPState::IDLE;
  deferred_hp_state_ = HPState::UNKNOWN;
...
void OpenAmberComponent::SetNextState(HPState new_state)
{
  hp_state_ = new_state;
...
void OpenAmberComponent::LeaveStateAndSetNextStateAfterWaitTime(HPState new_state, uint32_t defer_ms)
{
  deferred_hp_state_ = new_state;
  defer_state_change_until_ms_ = millis() + defer_ms;
  SetNextState(HPState::WAIT_FOR_STATE_SWITCH);
}

void OpenAmberComponent::DoSafetyChecks()
{
...
void OpenAmberComponent::UpdateStateMachine()
{
  ...
  case HPState::WAIT_FOR_STATE_SWITCH:
  {
    ...
    {
      defer_state_change_until_ms_ = 0;
      SetNextState(deferred_hp_state_);
      deferred_hp_state_ = HPState::UNKNOWN;
      break;
    }
  }
  case HPState::IDLE:
  {
    ...
    if (dhw_demand && !valve_controller_->IsInPosition(ThreeWayValvePosition::DHW))
    {
      valve_controller_->SetPosition(ThreeWayValvePosition::DHW);
      is_switching_modes_ = true;
      LeaveStateAndSetNextStateAfterWaitTime(HPState::IDLE, THREE_WAY_VALVE_SWITCH_TIME_S * 1000UL);
      break;
    }
    else if (!dhw_demand && !valve_controller_->IsInPosition(ThreeWayValvePosition::HEATING_COOLING))
    {
      valve_controller_->SetPosition(ThreeWayValvePosition::HEATING_COOLING);
      is_switching_modes_ = true;
      LeaveStateAndSetNextStateAfterWaitTime(HPState::IDLE, THREE_WAY_VALVE_SWITCH_TIME_S * 1000UL);
      break;
    }
    ...
    if (now >= pump_controller_->GetNextPumpCycle() || compressor_controller_->ShouldStart(pump_controller_->GetPumpStartTime()))
    {
  ...
  {
    bool should_start_compressor = compressor_controller_->ShouldStart(pump_controller_->GetPumpStartTime());
    pump_controller_->ApplySpeedChangeIfNeeded();

    // Stop pump when duration expired
    if (pump_controller_->IsIntervalCycleFinished() && !should_start_compressor)
    {
    ...
    bool pump_on_long_enough = millis() - pump_controller_->GetPumpStartTime() >= (COMPRESSOR_MIN_TIME_PUMP_ON * 1000UL);
    if (!pump_on_long_enough && !valve_controller_->IsInDhwMode())
    {
      ESP_LOGI("amber", "Not starting compressor because Tc needs to stabilize (pump on time too short)");
      break;
    }

    if (!compressor_controller_->HasPassedMinOffTime(is_switching_modes_))
    {
      ESP_LOGI("amber", "Not starting compressor because minimum compressor time off is not reached.");
      break;
    }

    int workingMode = compressor_controller_->DetermineWorkingMode();
    auto working_mode_call = id(working_mode_switch).make_call();
    working_mode_call.set_index(workingMode);
    working_mode_call.perform();
    
    compressor_controller_->Start(is_switching_modes_);
    is_switching_modes_ = false;
  ...
  case HPState::COMPRESSOR_RUNNING:
  {
    ...
    if (compressor_controller_->HasPassedMinOnTime())
    {
      ...
      float target_temperature = valve_controller_->IsInDhwMode() ? id(current_dhw_setpoint_sensor).state : id(pid_heat_cool_temperature_control).target_temperature;
      float current_temperature = valve_controller_->IsInDhwMode() ? id(dhw_temperature_tw_sensor).state : id(heat_cool_temperature_tc).state;
...
void OpenAmberComponent::write_heat_pid_value(float pid_value)
{
...
// ============================================================================
// YAML LAMBDA WRAPPER FUNCTIONS
// ============================================================================
// These functions are called from YAML lambdas and forward to the component

inline void OpenAmberWriteHeatPIDValue(float pid_value)
{
  id(amber_ctrl).write_heat_pid_value(pid_value);
}

inline void OpenAmberResetPumpInterval()
{
  id(amber_ctrl).reset_pump_interval();
}
